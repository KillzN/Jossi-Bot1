...
/* Creditos a Otosaka (https://wa.me/50764880277) */
...
const connectionOptions = {
    ...
    mobile: MethodMobile,
    browser: opcion === '1' ? ['𝐉𝐎𝐒𝐒𝐈 𝐁𝐎𝐓 ', 'Safari', '2.0.0'] : methodCodeQR ? ['𝐉𝐎𝐒𝐒𝐈 𝐁𝐎𝐓 ', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
    auth: {
        creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./KillzNSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./KillzNSession/${files}`)
})
} 
...
const directories = ['./KillzNSession/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./KillzNSession/" + "creds.json")
